...
module.exports = () => {
  describe('URL/request method inputs', () => {
    ...
    it('can select a request type', async () => {
      ...
      assert.strictEqual(await composerObj.selectedNetwork.getText(), "GRAPHQL");

      // click and check WS
      await composerObj.selectedNetwork.click();
      await app.client.$('a=WEB SOCKETS').click();
      assert.strictEqual(await composerObj.selectedNetwork.getText(), "WEB SOCKETS");
      ...
      return assert.strictEqual(await composerObj.selectedNetwork.getText(), "REST");
    });
    ...
    it('can type url into url input', async () => {
      await composerObj.url.setValue('http://jsonplaceholder.typicode.com/posts/1');
  ...
  describe('cookies inputs', async () => {
    ...
    it('can add new cookies in request and type in keys & values', async () => {
      ...
      await app.client.$('//*[@id="cookie-row1"]/input[1]').setValue('cookie-key');
      await app.client.$('//*[@id="cookie-row1"]/input[2]').setValue('cookie-value');
      
      // select 2nd cookie key / value
      const cookieKey = await app.client.$('//*[@id="cookie-row1"]/input[1]').getValue();
      const cookieValue = await app.client.$('//*[@id="cookie-row1"]/input[2]').getValue();
  ...
  describe('request body inputs', () => {
    ...
    it('can type plain text into body', async () => {
      ...
      assert.strictEqual(await app.client.$('#body-entry-select .CodeMirror-line').getText(), input);
    });
  });
