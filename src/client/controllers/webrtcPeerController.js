...
export default class Peer {
  ...
  _initICECandidateEvents(){
    ...
    this.connection.onicecandidate = event => { 
      if (event && event.target && event.target.iceGatheringState === 'complete') {
        ...
        console.log(event.target.iceGatheringState, event, this.connection.localDescription);
  ...
  initDataChannelAndEvents(){
    ...
    if (this.role === this._roles.PENDING){
      ...
      dataChannel.onmessage = event => console.log("Received Msg: " + event.data);
        ...
        this.connection.ondatachannel = event => {
          ...
          this.connection.dataChannel.onopen = event => console.log("Connection opened!");
          // when the channel is closed ...
          //remoteConnection.dataChannel.onclose = event => console.log("Connection closed! Goodbye (^-^)");
          this.connection.dataChannel.onclose = event => console.log("Connection closed! Goodbye (^-^)");
          // when message received...
          //remoteConnection.dataChannel.onmessage = event => console.log("PeerA: " + event.data);
          this.connection.dataChannel.onmessage = event => console.log("Received Msg" + event.data);
        }
    }
  }
  ...
  createLocalSdp(){
    ...
    if (this.role === this._roles.INITIATOR){
      this.connection.createOffer()
        ...
        .catch(`[webrtcPeerControler][createLocalSdp] ERROR while attempting to set offer`);
    }
  }
