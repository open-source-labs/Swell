...
class GraphQLBodyEntryForm extends Component {
  ...
  handleKeyPress(event) {
    if (event.key === "Tab") {
      ...
      const gqlBodyEntryTextArea = document.querySelector(
        "#gqlBodyEntryTextArea"
      );
      const start = gqlBodyEntryTextArea.selectionStart;
      const second = gqlBodyEntryTextArea.value.substring(
        gqlBodyEntryTextArea.selectionStart
      );
      ...
      // })
      gqlBodyEntryTextArea.value =
        gqlBodyEntryTextArea.value.substring(0, start) +
        `  ` +
        gqlBodyEntryTextArea.value.substring(start);
      gqlBodyEntryTextArea.setSelectionRange(
        gqlBodyEntryTextArea.value.length - second.length,
        gqlBodyEntryTextArea.value.length - second.length
      );
    }
  }
  ...
  render() {
    ...
    return (
      <div>
        ...
        ></textarea>
        {this.props.warningMessage.body && (
          <div className="warningMessage">{this.props.warningMessage.body}</div>
        )}
