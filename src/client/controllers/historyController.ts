    ...
    db.table('history')
      ...
      .then((history: NewRequestResponseObject[]) => {
        const historyGroupsObj = history.reduce(
          (
            groups: Record<string, NewRequestResponseObject[]>,
            hist: NewRequestResponseObject
          ) => {
        ...
        const historyGroupsArr = Object.keys(historyGroupsObj)
          ...
          .map((date: string) => ({
            // this returns an array of objects with the date as the key and the array of history objects as the value
            date,
            history: historyGroupsObj[date].sort(
              (a: NewRequestResponseObject, b: NewRequestResponseObject) =>
