...
const ReqResCtrl = {
  ...
  fetchController(parsedObj, url, originalObj, abortController) {
    ...
    return fetch(url, parsedObj)
      .then(response => {
        // console.log('ReqResCtrl fetch>>>>>>>>>>>>>>',response);
        let heads = {};
        ...
        isStream
          ? this.handleSSE(response, originalObj, timeSentSnap, heads) // jshint ignore: line
    ...
    function read() {
      reader.read().then(obj => {
        ...
        //decode and recursively call
          ...
          let receivedEventFields = new TextDecoder("utf-8")
            .decode(obj.value)
          ...
          // console.log('read obj >>>>>>>>>',receivedEventFields);

          //newObj. something is filling in the nulls
          newObj.response.events.push(receivedEventFields);

          //dispatch sends it back to the store
          store.default.dispatch(actions.reqResUpdate(newObj));
          read();
        }
      });
